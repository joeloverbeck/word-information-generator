...
def get_word_info(word):
    ...
    try:
        ...
        with open(output_filepath, "w", encoding='utf-8') as f:
            ...
            write_section_to_file(f, f"Part of speech for {word}", parts_of_speech_strings)
            write_section_to_file(f, f"Etymology of {word}", [etymology_result] if etymology_result else [])
            ...
            write_section_to_file(f, f"Word frequencies", [f"{w}: {freq}" for w, freq in word_frequencies.items()], use_table=True)
            write_section_to_file(f, f"Collocations for {word} and its synonyms", [f"{' '.join(bigram)} (Frequency: {freq})" for bigram, freq in collocations])
            write_section_to_file(f, f"Phrasal verbs of {word}", phrasal_verbs)
            write_section_to_file(f, f"Idiomatic expressions with {word}", idiomatic_expressions)
            write_section_to_file(f, f"Related phrases and expressions with {word}", related_phrases_and_expressions)
            write_section_to_file(f, f"Semantic field(s) of {word}", semantic_fields)
            write_section_to_file(f, f"Hyponyms of {word}", [synset.split('.')[0] for synset in hyponyms], use_table=True)
            write_section_to_file(f, f"Hypernyms of {word}", [synset.split('.')[0] for synset in hypernyms], use_table=True)
            write_section_to_file(f, f"Meronyms of {word}", [synset.split('.')[0] for synset in meronyms], use_table=True)
            write_section_to_file(f, f"Domain-specific words related to {word}", [word for word in domain_words])
            write_section_to_file(f, f"Alternative words for {word}", alternative_words)        
            write_section_to_file(f, f"Associated nouns with {word}", associated_nouns, use_table=True)
            write_section_to_file(f, f"Associated verbs with {word}", associated_verbs, use_table=True)      
            write_section_to_file(f, f"Morphological variations of {word}", morphological_variations)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Get detailed information about a given word.")
