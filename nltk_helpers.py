...
def get_word_frequencies(words_list):
    ...
    if not all(isinstance(word, str) for word in words_list):
        raise ValueError(f"The function 'get_word_frequencies' received a list of words that didn't contain only strings: {words_list}")
...
def get_collocations(words_list):
    ...
    collocations = [bigram for bigram in top_collocations if any(word in bigram for word in words_list)]
