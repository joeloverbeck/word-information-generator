...
class TestWordnetUtils(unittest.TestCase):
    def test_get_synonyms_antonyms_white(self):
        ...
        self.assertTrue(
            expected_synonyms.issubset(synonyms),
            f"Expected synonyms not found: {expected_synonyms - synonyms}",
        )
        self.assertTrue(
            expected_antonyms.issubset(antonyms),
            f"Expected antonyms not found: {expected_antonyms - antonyms}",
        )
    ...
    def test_get_associated_nouns_verbs(self):
        ...
        self.assertTrue(
            expected_nouns.issubset(nouns),
            f"Expected nouns not found: {expected_nouns - nouns}",
        )
        self.assertTrue(
            expected_verbs.issubset(verbs),
            f"Expected verbs not found: {expected_verbs - verbs}",
        )
